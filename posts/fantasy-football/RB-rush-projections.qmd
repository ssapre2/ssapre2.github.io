...
library(tidymodels)
# Split the data
splits <- rsample::initial_split(model_data, prop = 0.8, strata = age)
...
As you saw, I took the lag value of several variables making our initial projections (rookie year) unpredictable at this point. However, I think we can reasonably predict how many carries a rookie may receive in his rookie year given a simple formula.

```{r}
rb_train |>
  filter(years_exp == 0) -> rookie_train

rookie_fit <- lm(carries ~ season + draft_number, data = rookie_train)

broom::glance(rookie_fit)

tidy(rookie_fit, conf.int = TRUE)
```
