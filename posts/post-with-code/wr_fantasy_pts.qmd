...
# Residuals vs fitted
ggplot(glm_predictions,aes(x = .pred, y=.resid)) +
  geom_point() +
...
This plot is a bit indicting as we can see the points at the end of the QQPlot start to tail off, suggesting that there are some outliers in fantasy points scored or a linear model may not be the best approach for this dataset and we probably should not trust this model for valid inference and prediction. We should use this as a reason to explore non-linear modeling techniques or modify this model to include interactions or transform the target variable.  
...
As mentioned earlier, we will probably need to better account for interaction terms either thr
